	...
	if (use_index) {
		related = matchingIndexes.filter((index) => {
			if (typeof use_index === 'string') {
				...
				return (
					index.ddoc.split('/')[1] === use_index[0] &&
					index.name === use_index[1]
				)
			} else {
		...
		related = matchingIndexes.filter(
			(index) => index.def.partial_filter_selector === undefined
		)
	}
	if (related.length) {
		return related[0]
