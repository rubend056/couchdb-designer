	...
	if (aType === bType) {
		switch (aType) {
			...
			case 'array':
				return (
					a.length === b.length &&
					a.every((value, index) => jokerEquals(value, b[index]))
				)
			case 'object':
				...
				return (
					...
					aKeys.every((key) => jokerEquals(a[key], b[key]))
				)
		}
	} else {
		return false
...
const validateViewOptions = (hasReduce, options = {}) => {
	...
	let group =
		options.group === undefined ? false || group_level > 0 : options.group
	if (!reduce && group) {
	...
	return { reduce, group, group_level }
}

module.exports = { validateViewOptions, jokerEquals }
