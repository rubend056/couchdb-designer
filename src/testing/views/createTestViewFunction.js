...
const createTestViewFunction = (contextId, viewName, context) => {
	return (opts, partition) => {
		...
		if (context.views[viewName].map) {
			let options = validateViewOptions(
				Boolean(context.views[viewName].reduce),
				opts
			)
			let viewResult
			if (
				(partition && context.options && context.options.partitioned) ||
				(partition && !context.options)
			) {
				database.data
					.filter((doc) => {
				...
				if (partition) {
					...
					if (
						(context.options && context.options.partitioned) ||
						(database.partitioned && !context.options)
					) {
						throw {
							error: 'query_parse_error',
							reason:
