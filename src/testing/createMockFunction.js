...
const callingErrors = {
	...
	Require:
		"Calling 'require' from reduce function in is not allowed and useless from library!",
	GetRow: "Calling 'getRow' allows only in list functions!",
	Provides: "Calling 'provides' allows only in list and show functions!",
	RegisterType:
		"Calling 'registerType' allows only in list and show functions!",
...
const createMockFunction = (fileStats, contextProps, name, designFunction) => {
	...
	const mockFunction = (...args) => {
		for (let allowed of policy.allowed) {
			if (allowed === 'Emit') {
				...
				buildIns.environmentRequire.mockImplementation((requirePath) => {
					if (
						policy.allowed.includes('Emit') &&
						requirePath.indexOf('views') !== 0
					) {
						throw {
				...
				buildIns[`environment${allowed}`].mockImplementation((...builtInArgs) =>
					buildIns[`contexted${allowed}`](...builtInArgs)
				)
			}
		}
		...
		if (policy.uri) {
			args[1] = supplementRequest(
				args[1],
				null,
				contextProps.contextId,
				policy.uri
			)
		}
		let result
		if (fileStats.isLib) {
			try {
				...
				if (err.testError) {
					throw new Error(
						`${callingErrors[err.errorType]} You can fix it in ${
							fileStats.filePath
						}`
					)
				} else {
					throw new Error(
						`Your library ${fileStats.filePath} throw error: ${err.message}`
					)
				}
			}
		} else {
		...
		if (fileStats.isLib) {
			buildIns.environmentRequire.mockImplementation((requirePath) =>
				buildIns.contextedRequire(requirePath)
			)
		}
		return result
	}
