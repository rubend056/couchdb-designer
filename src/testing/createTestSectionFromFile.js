...
const creteTestSectionFromFile = (directory, fileName, contextProps) => {
	return new Promise((resolve, reject) => {
		...
		fs.readFile(fileStats.filePath, { encoding: 'utf8' }).then(
			(content) => {
				if (!fileStats.isJavaScript) {
					...
					createTestJavascriptSection(fileStats, contextProps, content).then(
						resolve,
						reject
					)
				}
			...
			(err) =>
				reject(
					`Bad structure! ${fileStats.filePath} must be regular file! ${err.message}`
				)
		)
	})
}
