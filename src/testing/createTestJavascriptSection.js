...
const createTestJavascriptSection = (fileStats, contextProps, fileContent) => {
	return new Promise((resolve, reject) => {
		try {
			const testModule = createTestJavascriptModule(
			...
			if (testModule && Object.keys(testModule).length > 0) {
				...
				if (
					!fileStats.isLib &&
					testModuleKeys.length === 1 &&
					testModuleKeys[0] === fileStats.name
				) {
					const mockFunction = createMockFunction(
						fileStats,
						contextProps,
						testModuleKeys[0],
						testModule[testModuleKeys[0]]
					)
					...
					for (let moduleElementName of testModuleKeys) {
						if (typeof testModule[moduleElementName] === 'function') {
							const mockFunction = createMockFunction(
								...
								contextProps,
								moduleElementName,
								testModule[moduleElementName]
							)
							testElementsObject[moduleElementName] = mockFunction
						} else {
							testElementsObject[moduleElementName] =
								testModule[moduleElementName]
						}
					}
				...
				reject(
					`The module ${fileStats.filePath} does not exist't export anything! You must export function/s with module.exports = {...}`
				)
			}
		} catch (moduleError) {
