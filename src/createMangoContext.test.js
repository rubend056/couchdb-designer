...
const jsonSuccess = JSON.stringify({
	...
	mongo: {
		...
		fields: ['_id', { _rev: 'asc' }, 'year', 'title'],
	},
})
...
const jsonWrongSort1 = JSON.stringify({
	...
	mongo: {
		...
		fields: ['_id', { _rev: 'desc' }, 'year', 'title'],
	},
})
...
const jsonWrongSort2 = JSON.stringify({
	...
	mongo: {
		...
		fields: [{ _rev: 'desc' }, '_id', 'year', 'title'],
	},
})
...
const jsonInvalidSort1 = JSON.stringify({
	...
	mongo: {
		...
		fields: [{ _rev: 'desac' }, '_id', 'year', 'title'],
	},
})
...
const jsonInvalidSort3 = JSON.stringify({
	...
	mongo: {
		...
		fields: [{ helo: 'asc', szia: 'desc' }, '_id', 'year', 'title'],
	},
})
...
describe('createMangoDocument', () => {
	...
	test('success', () => {
		...
		return createMangoContext('root', 'name', true).then((context) => {
			...
			expect(context.views.mongo.options.def).toEqual(
				JSON.parse(jsonSuccess).mongo
			)
		})
	})
	...
	test('invalid partitioned', () => {
		...
		return createMangoContext('root', 'name', false).catch((error) => {
			expect(error).toBe(
				'You cannot create partitioned indexes in a non partitioned database! You can fix it in root/name.'
			)
		})
	})
	...
	test('wrong sort1', () => {
		...
		return createMangoContext('root', 'name', true).catch((error) => {
			expect(error).toBe(
				'(unsupported_mixed_sort) Sorts currently only support a single direction for all fields. You can fix it in root/name - index: mongo.'
			)
		})
	})
	...
	test('invalid sort1', () => {
		...
		return createMangoContext('root', 'name', true).catch((error) => {
			expect(error).toBe(
				"(invalid_sort_field) Invalid sort field: { _rev: 'desac' }. You can fix it in root/name,mongo index."
			)
		})
	})
	...
	test('invalid sort2', () => {
		...
		return createMangoContext('root', 'name', true).catch((error) => {
			expect(error).toBe(
				'(invalid_sort_field) Invalid sort field: true. You can fix it in root/name,mongo index.'
			)
		})
	})
	...
	test('invalid sort3', () => {
		...
		return createMangoContext('root', 'name', true).catch((error) => {
			expect(error).toBe(
				"(invalid_sort_field) Invalid sort field: { helo: 'asc', szia: 'desc' }. You can fix it in root/name,mongo index."
			)
		})
	})
})
