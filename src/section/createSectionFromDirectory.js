...
const createSectionFromDirectory = (
	directory,
	sectionName,
	contextProps = false
) => {
	return new Promise((resolve, reject) => {
		...
		fs.readdir(directoryPath).then(
			(names) => {
				Promise.all(
					names.map((name) => {
						return createSection(directoryPath, name, contextProps)
					})
				).then(
					(sections) => {
						...
						for (let section of sections) {
							if (directorySection.hasOwnProperty(Object.keys(section)[0])) {
								...
								directorySection[sectionKey] = Object.assign(
									directorySection[sectionKey],
									section[sectionKey]
								)
							} else {
			...
			(err) =>
				reject(
					`Bad structure! ${directoryPath} must be a directory! ${err.message}`
				)
		)
	})
}
