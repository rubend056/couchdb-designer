...
export default function createTestContext(
	...
	return new Promise((resolve, reject) => {
		...
		createSectionFromDirectory(directory, name, contextProps).then(
			(section) => {
				testContext = Object.assign(testContext, section[name])
				if (
					!isDatabasePatritioned &&
					testContext.options &&
					testContext.options.partitioned
				) {
					reject(
						'partitioned option cannot be true in a non-partitioned database.'
					)
				} else {
					if (testContext.language.toLowerCase() === 'javascript') {
						if (testContext.validate_doc_update) {
							addValidator(
								contextId,
								testContext.id,
								testContext.validate_doc_update
							)
						}
						...
						if (testContext.language.toLowerCase() === 'query') {
							reject(
								'Warning! You can testing Mango index with createTestServer by defining them in a json file under its root directory.'
							)
						} else {
							reject(
								`Only "javascript" type design document testing is supported yet. This directory structure defining one "${testContext.language}" type design document!`
							)
						}
					}
				}
