	...
	return new Promise((resolve, reject) => {
		...
		fs.readFile(filePath).then((fileContent) => {
			try {
				...
				let keys = Object.keys(mangoJson).filter(
					(field) => field !== 'partitioned'
				)
				...
				Promise.all(
					keys.map((indexName) =>
						createMangoIndex(mangoJson[indexName], indexName, contextProps)
					)
				).then((views) => {
					...
					for (let view of views) {
						...
						indexes.push({
							...
							name: indexName,
							type: 'json',
							partitioned: mangoJson.partitioned
								? mangoJson.partitioned
								: false,
